...
def get_data(datatype):
    ...
    elif datatype == 'sin_freq':
        generate_data = sin_with_frequency
    else:
...
def sin_with_frequency(x, omega=16):
    return torch.sin(omega * x) + 0.1 * torch.rand_like(x)
...
def fractal_function(x):
    x_1 = x[:, 0]
    x_2 = x[:, 1]
    z = torch.sin(10 * math.pi * x_1) * torch.cos(10 * math.pi * x_2) + torch.sin(math.pi * torch.sum(x ** 2, axis=1))
    z += torch.abs(x_1 - x_2) + (torch.sin(5 * x_1 * x_2) / (0.1 + torch.abs(x_1 + x_2)))
    z *= torch.exp(-0.1 * torch.sum(x ** 2,axis=1))
    return z[:, None]

def multimodal_function1(x):
    x_1 = x[:, 0]
    x_2 = x[:, 1]
    z = -torch.abs(torch.sin(x_1) * torch.cos(x_2) * torch.exp(torch.abs(1 - (torch.sqrt(torch.sum(x ** 2, axis=1)) / math.pi))))
    return z[:, None]  # Reshape to column vector


def multimodal_function2(x, alpha=1):
    x_1 = x[:, 0]
    x_2 = x[:, 1]
    z = (-20.0 * torch.exp(-0.2 * torch.sqrt(alpha * torch.sum(x ** 2, axis=1))) -
         torch.exp(alpha * (torch.cos(2 * math.pi * x_1) + torch.cos(2 * math.pi * x_2))) +
         math.e + 20)
    return z[:, None]
...
def function_4D(x, alpha=1): # x is of batch size * 4  alpha = 0.5
    ...
    z = torch.exp(alpha * torch.sin(math.pi * torch.sum(x1_x2 ** 2, axis=1)) +
               alpha * torch.sin(math.pi * torch.sum(x3_x4 ** 2, axis=1)))[:, None]
...
def function_100D_smooth(x, alpha=0.001): #used alpha = 0.001
    print("x type:", type(x))
    print("x content:", x)

    z = torch.exp(-alpha * torch.sum(x ** 2,axis=1))[:,None] #[:, None] is used to convert a 1D array with shape (1000,) into a 2D column vector with shape (1000, 1). This is done to ensure that the array has the correct shape for subsequent operations, especially when performing element-wise multiplication and broadcasting.

    return z

def function_100D_osc(x, alpha=0.001):
    print("x type:", type(x))
    print("x content:", x)

    z = torch.exp(-alpha * torch.sum(torch.sin(x ** 2),axis=1))[:,None] #[:, None] is used to convert a 1D array with shape (1000,) into a 2D column vector with shape (1000, 1). This is done to ensure that the array has the correct shape for subsequent operations, especially when performing element-wise multiplication and broadcasting.
